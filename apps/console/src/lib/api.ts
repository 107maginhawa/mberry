 ...
 * and any raw fetch calls so they can never drift.
 */
export const API_BASE = import.meta.env.VITE_API_URL ?? `${window.location.origin}/api`

/**
 * Configure the shared SDK client for the authed platform-operator console:
 ...
 *           recovers the rare mid-session expiry.
 * NOTE: NO x-org-id injection — console is not org-scoped (admin/* endpoints).
 */
