...
/**
 * Configure the shared SDK client for the authed member app:
...
export function configureApiClient(baseUrl = API_BASE): void {
  ...
  client.interceptors.request.use(async (request: Request) => {
    ...
    // Org-scoped read endpoints gate on the x-org-id header (org-context.ts).
    // Inject the selected org only on org-scoped paths (not /auth or /pay public flows).
    if (!isExemptPath(pathname)) {
      const orgId = localStorage.getItem('member.selectedOrgId')
