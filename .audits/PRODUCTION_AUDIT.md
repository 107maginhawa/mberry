...
5. ~~Move audit + officer-check to middleware (cuts 343 import sites)~~ **Fully resolved 2026-06-06.** Per-route audit middleware (`@extension("x-audit", ...)`) + officer/position middleware (`@extension("x-require-officer", ...)` / `@extension("x-require-position", ...)`) wired through TypeSpec → generator → routes.ts. Final state: `utils/audit.ts` deleted (helper relocated to `core/audit/audit-action.ts` for the 16 hand-wired-in-`app.ts` handlers); `utils/officer-check.ts` deleted (helper relocated to `core/auth/officer-checks.ts` for ~60 remaining handlers in `association:member/` where authorization branches on runtime state and can't be expressed as a static `@extension`). All migratable handlers (4 Bucket-A multi-event audit + 2 Bucket-D single-event audit + 32 ops position-checks + 4 small-module position-checks) now declarative. Per-route audit middleware also gained try/finally so multi-event entries persist on handler throw. See CLAUDE.md for the new pattern. Original Bucket F premise (entity-derived orgId middleware) recon-disproven and infrastructure reverted. Follow-up: 60 `association:member/` handlers eligible for case-by-case migration. Commits: c23eaed2, 0f97af53, 93bbc935, 56deb57e, c8edeb36, 68c1b495, f1135c5e, ade77d2a, ddd3f909, 111b73ec.
6. ~~Convert `accountDeletionCascade` to event-driven per-module subscribers~~ **Fully resolved 2026-06-06.** `person.deleted` registered in `core/domain-events.registry.ts`. 9 subscribers in `core/domain-event-consumers.ts` reproduce the 19-step cascade across `association:member` (9 tables), `association:operations` (events + training), `elections`, `certificates`, `communication`, `documents`, `invite`, `billing`, and `person`. `accountDeletionCascade.ts` collapsed from 436 → 46 LOC — single `domainEvents.emit('person.deleted', ...)` call + new `CascadeResult = { emitted, personId, emittedAt }`. Callers (`executeAccountDeletion`, `jobs/deletionProcessor`) dropped per-step error aggregation; subscribers own their own try/catch + logger.error. Fire-and-forget semantics. Subscriber-level behavioral tests deferred (TODO marked in `accountDeletionCascade.test.ts`). Commits: 182c1369, 26b00642, 10a782a0.
