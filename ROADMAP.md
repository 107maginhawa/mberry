...
| BR-37 | Job Posting Expiry | 2 | M15 | 30-day expiry with reminders and extensions — **SHIPPED (2026-06-22)**: expiry cron (`jobs.expirePostings`, active→expired past expiry) + public-board exclusion of expired (officer history via `status=expired`) + extend endpoint `POST /association/jobs/postings/{id}/extend` (resets +30d from current expiry, reactivates) + 3-day reminder cron (date-band dedup, no migration). Real-PG + live-verified. Still deferred: contract/e2e + job-board FE + dedicated notif type. |
