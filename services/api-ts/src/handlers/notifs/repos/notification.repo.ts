  ...
  private async deliverNotification(notification: Notification): Promise<void> {
    ...
    switch (notification.channel) {
      case 'email': {
        ...
        const emailService = (globalThis as unknown as Record<string, Record<string, unknown>>).app?.email;
        if (emailService) {
          ...
          if (templateTag) {
            ...
            if (person && (person as Record<string, unknown>).email) {
              await emailService.queueEmail({
                ...
                recipient: (person as Record<string, unknown>).email as string,
                variables: {
      ...
      case 'push':
        ...
        if (this.oneSignalClient && this.oneSignalAppId) {
          try {
            ...
            const targetApp = (notification as Record<string, Record<string, string>>).data?.targetApp;
            if (targetApp) {
