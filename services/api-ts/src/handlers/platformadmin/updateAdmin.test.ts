...
describe('updateAdmin', () => {
  ...
  test('returns 403 when caller is not super (support)', async () => {
    const ctx = makeCtx({ platformAdmin: { id: 'pa-1', role: 'support' }, _params: { adminId: 'admin-1' }, _body: { role: 'analyst' } });
    const res = await updateAdmin(ctx);
    expect(res.status).toBe(403);
  });

  test('returns 200 with updated admin', async () => {
    const ctx = makeCtx({ platformAdmin: { id: 'pa-1', role: 'super' }, _params: { adminId: 'admin-1' }, _body: { role: 'analyst' } });
    const res = await updateAdmin(ctx);
  ...
  test('throws NotFoundError when admin not found', async () => {
    ...
    const ctx = makeCtx({ platformAdmin: { id: 'pa-1', role: 'super' }, _params: { adminId: 'nonexistent' }, _body: {} });
  ...
  test('throws BusinessLogicError when demoting last super admin', async () => {
    ...
    const ctx = makeCtx({ platformAdmin: { id: 'pa-1', role: 'super' }, _params: { adminId: 'admin-1' }, _body: { role: 'support' } });
  ...
  test('allows updating super admin without role change', async () => {
    ...
    const ctx = makeCtx({ platformAdmin: { id: 'pa-1', role: 'super' }, _params: { adminId: 'admin-1' }, _body: { name: 'New Name' } });
    const res = await updateAdmin(ctx);
    expect(res.status).toBe(200);
  });
});
