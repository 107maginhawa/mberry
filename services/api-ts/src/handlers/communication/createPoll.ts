...
export async function createPoll(ctx: Context): Promise<Response> {
  ...
  if (!body.title || String(body.title).trim().length === 0) {
    return ctx.json({ error: 'Poll title is required' }, 400);
  }

  const questions = body.questions;
  if (!Array.isArray(questions) || questions.length === 0) {
    return ctx.json({ error: 'Poll must have at least one question' }, 400);
  }

  // Poll must have exactly one multiple-choice question
  ...
  const poll = await repo.create({
    ...
    showResultsImmediately: true, // polls always show results immediately
    reminderSchedule: body.reminderSchedule ?? [],
    createdBy: session.user.id,
    updatedBy: session.user.id,
  });

  return ctx.json({ data: poll }, 201);
}
