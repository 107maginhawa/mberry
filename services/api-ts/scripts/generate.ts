...
async function generateValidators(spec: any) {
  ...
  await writeFile(path.join(GENERATED_DIR, 'validators.ts'), DO_NOT_EDIT_HEADER + validators.join('\n'));
...
async function generateRegistry(paths: Record<string, PathItem>) {
  ...
  await writeFile(path.join(GENERATED_DIR, 'registry.ts'), DO_NOT_EDIT_HEADER + registry.join('\n'));
