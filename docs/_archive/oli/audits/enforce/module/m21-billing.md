# Module Enforcement Report: M21 — Billing

---
oli_version: "engine-v5 — Enforcement"
oli_artifact: ENFORCE_MODULE
Module: m21-billing
Spec: docs/product/modules/m21-billing/ (prose-only)
Code: services/api-ts/src/handlers/billing/ (18 .ts)
Contract: services/api-ts/src/handlers/billing/API_CONTRACTS.md
...
Module M21 (Billing) has **implemented code** (18 `.ts` handlers + `API_CONTRACTS.md`) but its spec under `docs/product/modules/m21-billing/` is **prose-only**: zero `BR-`, `AC-`, `WF-`, `SM-` anchor IDs.
...
| ID | Severity | Finding |
|----|----------|---------|
| EM-M21-zeroanchor | P3 | Spec prose-only — no BR-/AC-/WF-/SM- anchors. DECLARED_API empty; conformance dimensions N/A. |

## Dimension Status

| Dimension | Status | Reason |
|-----------|--------|--------|
| API Endpoints | N/A | No anchored endpoint declarations to match against 18 handlers |
...
Author an anchored MODULE_SPEC so conformance can be enforced against the existing 18-handler Stripe Connect implementation. Report-only INCOMPLETE at 7.0 cap until then. No P0/P1 emitted.
