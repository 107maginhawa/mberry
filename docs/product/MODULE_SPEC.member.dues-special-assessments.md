...
| Contract coverage of `Member/DuesSpecialAssessments` | **64% covered (32/50 ops)** after Wave 8 + Wave 9. Remaining gap: dues-config CRUD (create/get/list/update/delete), invoice CRUD (create/list/get/update/delete/mark-paid/generate-org), payment proof/confirm/reject, refund flow, special-assessment update/delete/list. |
| `upsertDuesGatewayConfig` secret encryption | **FIXED** (Wave 9, commit pending) field-name mismatch — handler now maps `body.secretKey` → `encryptedSecret` column. **Plaintext-at-rest** for now; real encryption is a future security task. |
| `upsertDuesFunds` field mapping | **FIXED** (Wave 9, commit pending) — handler now maps `body.funds[i].fundName` → `name`, derives `sortOrder` from array position. |
...
| Coverage strength caveat | `contract-coverage-gap.ts` counts operationId mentions in any Hurl file, not assertion strength. Reviewers should spot-check new Hurl files prefer `HTTP 200`/`HTTP 201` over `HTTP * + status < 600` — the latter masks 500s and inflates coverage numbers without testing correctness. |
