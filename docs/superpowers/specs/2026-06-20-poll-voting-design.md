...
- **Tenant boundary = any-status membership** — the member-survey endpoints (`findAvailableForMember`, `getSurvey`, `submitSurveyResponse`) scope by a membership row of *any* status (so a `lapsed`/`grace`/`pending` member can still read + vote on an active poll). Deliberate v1 choice (read-only poll access; vote dedup still holds). Tighten to active-only if a product rule requires it. Consistent across all three endpoints.
