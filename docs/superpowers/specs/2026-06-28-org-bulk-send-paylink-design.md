...
- **`features/roster/BulkResults.tsx`** — presentational replace-screen panel.
  - Props: `members`, `results`, `progress`, `onBack`. Per row: name + status
    icon + (sent → url + `Copy` button via `navigator.clipboard` + `toast`) /
    (no-dues → "No outstanding dues") / (error → message) / (pending|minting →
    spinner text). Manual-distribution note line. `Back to roster` button.
...
- `Roster.test.tsx` (extend) — Select toggle shows checkboxes + hides per-row
  link; Select-all picks filtered only; search drops selection; sticky bar count;
  send swaps to results; Back returns to list.
- `BulkResults.test.tsx` — renders each status variant; Copy copies url.
