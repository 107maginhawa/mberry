...
> - **billing-stripe Stripe fee/settlement path** — **BLOCKED `[NEEDS PRODUCT DECISION]` / `[BLOCKED BY MISSING SPEC]`.** The fee %, who-absorbs, refund-netting, and Connect model cannot be engineering-defaulted (no fee policy exists in m21; `core/billing.ts` is a `[SHARED DEPENDENCY]` for billing+dues+events). Founder must author a fee policy first. This is the ONLY remaining non-deferred blocker.
>
...
| Billing (Stripe) | billing-stripe | API/Integration | FAIL | Yes | Partially Fixed | A + **Batch B remainder (FIX-007/008)** done. Batch C/D remain; stripe-mock CI env-blocked. |
