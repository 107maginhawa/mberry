...
| --- | --- | --- | --- |
| Platform fee calculation (`platformAmount = 0`) | §9, §23 | V2 DEFERRED | Explicitly deferred in code (`payInvoice.ts:132`); no V1 revenue requirement. **Roadmap "Stripe-fee path" carry-forward ran a `04` scoping pass 2026-06-13 → BLOCKED** (`[BLOCKED BY MISSING SPEC]` fee policy + `[NEEDS PRODUCT DECISION]` V1 Connect model; shared `core/billing.ts` consumed by dues+events). See `billing-stripe-fix-report.md` § "Stripe Fee / Settlement Path — carry-forward scoping pass (2026-06-13)". Needs founder fee-policy + Connect-model decisions before any future `04`. |
